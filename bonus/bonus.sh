...
sort -t';' -k2 tmp/bonus.txt > demo/bonus.txt
...
plot "demo/bonus.txt" using (0.5*$2):0:(0.5*$2):(myBoxWidth/2.):($0+1):ytic(1) with boxxy lc var
### end of script
...
xdg-open "demo/bonus.png"
echo "██████████ 100%"
end=$(echo $SECONDS );
rm "$gnuplot_script" tmp/tmp.csv tmp/bonus.txt
